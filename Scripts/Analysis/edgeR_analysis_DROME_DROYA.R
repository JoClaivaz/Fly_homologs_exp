...
modif_group = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/data_expression_DROME_DROYA', sep = '\t')
control_group = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/data_expression_DROME_DROYA_nomodif', sep = '\t')
...
anat_common = intersect(modif_group$anatomical_entity_name[modif_group$specie == 'DROME'], modif_group$anatomical_entity_name[modif_group$specie == 'DROYA'])
...
pair_modification = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/final_pair_DROME_DROYA_domain_loss_fbgn', sep = '\t')

levels(as.factor(reduced_data$sex_specie_status))

read1_drome_female = c()
read2_drome_female = c()
read1_droya_female = c()
read2_droya_female = c()
read1_drome_male = c()
read2_drome_male = c()
read1_droya_male = c()
read2_droya_male = c()
...
for (line_pair in 1:dim(pair_modification)[1]){
  if (grepl('DROME', pair_modification[line_pair,5])){
    ...
    read1_droya_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][1]
    read2_droya_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][1]
    read1_droya_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][2]
    read2_droya_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][2]
  }
  if (grepl('DROME', pair_modification[line_pair,6])){
    ...
    read1_droya_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][1]
    read2_droya_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][1]
    read1_droya_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][2]
    read2_droya_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][2]
  }
}

pair_modif_name = unite(pair_modification, 'ortho_pair', c(V5, V6))
data_modification = data.frame(pair_modif_name$ortho_pair, read1_drome_female, read2_drome_female, read1_drome_male, read2_drome_male, read1_droya_female, read2_droya_female, read1_droya_male, read2_droya_male)

#group the different pair control
pair_control = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/ortholog_DROME_DROYA_domain_nomodif_fbgn', sep = '\t')
...
for (line_pair in 1:dim(pair_control)[1]){
  if (grepl('DROME', pair_control[line_pair,5])){
    ...
    read1_droya_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][1]
    read2_droya_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][1]
    read1_droya_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][2]
    read2_droya_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][2]
  }
  if (grepl('DROME', pair_control[line_pair,6])){
    ...
    read1_droya_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][1]
    read2_droya_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][1]
    read1_droya_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][2]
    read2_droya_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][2]
  }
}

pair_control_name = unite(pair_control, 'ortho_pair', c(V5, V6))
data_control = data.frame(pair_control_name$ortho_pair, read1_drome_female, read2_drome_female, read1_drome_male, read2_drome_male, read1_droya_female, read2_droya_female, read1_droya_male, read2_droya_male)
...
####edgeR####
######DGE LRT and QL DROMEvsDROYA / design with two factors: strain, sex (and replicates)######
species = c(rep('DROME', 4), rep('DROYA', 4))
...
pair_length_data = read.table(file = 'D:/UNIL/Master/Master_Project/Data/expression_analysis/DROME_DROYA_protein_length.txt', sep = '\t')
...
logFC_drome_droya_QL = rbind( female_modif, male_modif, female_control, male_control)
str(logFC_drome_droya_QL)
...
boxplot(logFC ~ status * sex, data = logFC_drome_droya_QL)

#condition anova
model_QL = aov(logFC ~ status * sex, data = logFC_drome_droya_QL)

qqnorm(residuals(model_QL)); qqline(residuals(model_QL))
plot(logFC_drome_droya_QL$logFC ~ logFC_drome_droya_QL$status * logFC_drome_droya_QL$sex)
#no normal distribution, ANOVA cant be applied

kruskal.test(logFC ~ status, data = logFC_drome_droya_QL)
kruskal.test(logFC ~ sex, data = logFC_drome_droya_QL)
...
pvalue_drome_droya_QL = rbind( female_modif_k, male_modif_k, female_control_k, male_control_k)
write.table(pvalue_drome_droya_QL, 'D:/UNIL/Master/Master_Project/Data/expression_analysis/all_droso/pvalue/pvalue_drome_droya_QL', sep = '\t')
####END p-value data
...
#enrichment test
resultFisher_droya_bp = runTest(topGO_droya_bp, algorithm = "classic", statistic = "fisher")
resultFisher_droya_bp

resultKS_droya_bp = runTest(topGO_droya_bp, algorithm = "classic", statistic = "ks")
resultKS.elim_droya_bp = runTest(topGO_droya_bp, algorithm = "elim", statistic = "ks")

#analysis
allRes_droya_bp = GenTable(topGO_droya_bp, classicFisher = resultFisher_droya_bp, 
                           classicKS = resultKS_droya_bp, elimKS = resultKS.elim_droya_bp,
                           orderBy = "elimKS", ranksOf = "classicFisher", topNodes = 25)

allRes_droya_bp

pValue.classic_droya = score(resultKS_droya_bp)
pValue.elim_droya = score(resultKS.elim_droya_bp)[names(pValue.classic_droya)]
gstat_droya = termStat(topGO_droya_bp, names(pValue.classic_droya))
gSize_droya = gstat_droya$Annotated / max(gstat_droya$Annotated) * 4
...
gCol_droya = colMap(gstat_droya$Significant)
plot(pValue.classic_droya, pValue.elim_droya, xlab = "p-value classic", ylab = "p-value elim",
     pch = 19, cex = gSize_droya, col = gCol_droya)

sel.go_droya = names(pValue.classic_droya)[pValue.elim_droya < pValue.classic_droya]
cbind(termStat(topGO_droya_bp, sel.go_droya),
      elim = pValue.elim_droya[sel.go_droya],
      classic = pValue.classic_droya[sel.go_droya])

showSigOfNodes(topGO_droya_bp, score(resultKS.elim_droya_bp), firstSigNodes = 5, useInfo = 'all')

#Molecular function
topGO_droya_mf = new("topGOdata", 
                     ontology = "MF", 
                     allGenes = all_genes, 
                     geneSel = genes_modif,
                     nodeSize = 10, 
...
#enrichment test
resultFisher_droya_mf = runTest(topGO_droya_mf, algorithm = "classic", statistic = "fisher")
resultFisher_droya_mf

resultKS_droya_mf = runTest(topGO_droya_mf, algorithm = "classic", statistic = "ks")
resultKS.elim_droya_mf = runTest(topGO_droya_mf, algorithm = "elim", statistic = "ks")

#analysis
allRes_droya_mf = GenTable(topGO_droya_mf, classicFisher = resultFisher_droya_mf, 
                           classicKS = resultKS_droya_mf, elimKS = resultKS.elim_droya_mf,
                           orderBy = "elimKS", ranksOf = "classicFisher", topNodes = 25)
