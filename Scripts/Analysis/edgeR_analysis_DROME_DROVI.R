...
modif_group = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/data_expression_DROME_DROVI', sep = '\t')
control_group = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/data_expression_DROME_DROVI_nomodif', sep = '\t')
...
anat_common = intersect(modif_group$anatomical_entity_name[modif_group$specie == 'DROME'], modif_group$anatomical_entity_name[modif_group$specie == 'DROVI'])
...
pair_modification = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/final_pair_DROME_DROVI_domain_loss_fbgn', sep = '\t')

levels(as.factor(reduced_data$sex_specie_status))

read1_drome_female = c()
read2_drome_female = c()
read1_drovi_female = c()
read2_drovi_female = c()
read1_drome_male = c()
read2_drome_male = c()
read1_drovi_male = c()
read2_drovi_male = c()
...
for (line_pair in 1:dim(pair_modification)[1]){
  if (grepl('DROME', pair_modification[line_pair,5])){
    ...
    read1_drovi_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][1]
    read2_drovi_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][1]
    read1_drovi_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][2]
    read2_drovi_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,1]][2]
  }
  if (grepl('DROME', pair_modification[line_pair,6])){
    ...
    read1_drovi_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][1]
    read2_drovi_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][1]
    read1_drovi_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][2]
    read2_drovi_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_modification[line_pair,3]][2]
  }
}

pair_modif_name = unite(pair_modification, 'ortho_pair', c(V5, V6))
data_modification = data.frame(pair_modif_name$ortho_pair, read1_drome_female, read2_drome_female, read1_drome_male, read2_drome_male, read1_drovi_female, read2_drovi_female, read1_drovi_male, read2_drovi_male)

#group the different pair control
pair_control = read.table('D:/UNIL/Master/Master_Project/Data/expression_analysis/ortholog_DROME_DROVI_domain_nomodif_fbgn', sep = '\t')
...
for (line_pair in 1:dim(pair_control)[1]){
  if (grepl('DROME', pair_control[line_pair,5])){
    ...
    read1_drovi_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][1]
    read2_drovi_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][1]
    read1_drovi_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][2]
    read2_drovi_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,1]][2]
  }
  if (grepl('DROME', pair_control[line_pair,6])){
    ...
    read1_drovi_female[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][1]
    read2_drovi_female[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][1]
    read1_drovi_male[line_pair] = reduced_data$read1[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][2]
    read2_drovi_male[line_pair] = reduced_data$read2[as.character(reduced_data$geneID_fbgn) == pair_control[line_pair,3]][2]
  }
}

pair_control_name = unite(pair_control, 'ortho_pair', c(V5, V6))
data_control = data.frame(pair_control_name$ortho_pair, read1_drome_female, read2_drome_female, read1_drome_male, read2_drome_male, read1_drovi_female, read2_drovi_female, read1_drovi_male, read2_drovi_male)
...
####edgeR####
######DGE LRT and QL DROMEvsDROVI / design with two factors: strain, sex (and replicates)######
species = c(rep('DROME', 4), rep('DROVI', 4))
...
pair_length_data = read.table(file = 'D:/UNIL/Master/Master_Project/Data/expression_analysis/DROME_DROVI_protein_length.txt', sep = '\t')
...
logFC_drome_drovi_QL = rbind( female_modif, male_modif, female_control, male_control)
str(logFC_drome_drovi_QL)
...
boxplot(logFC ~ status * sex, data = logFC_drome_drovi_QL)

#condition anova
model_QL = aov(logFC ~ status * sex, data = logFC_drome_drovi_QL)

qqnorm(residuals(model_QL)); qqline(residuals(model_QL))
plot(logFC_drome_drovi_QL$logFC ~ logFC_drome_drovi_QL$status * logFC_drome_drovi_QL$sex)
#no normal distribution, ANOVA cant be applied

kruskal.test(logFC ~ status, data = logFC_drome_drovi_QL)
kruskal.test(logFC ~ sex, data = logFC_drome_drovi_QL)
...
pvalue_drome_drovi_QL = rbind( female_modif_k, male_modif_k, female_control_k, male_control_k)
write.table(pvalue_drome_drovi_QL, 'D:/UNIL/Master/Master_Project/Data/expression_analysis/all_droso/pvalue/pvalue_drome_drovi_QL', sep = '\t')
####END p-value data
...
#enrichment test
resultFisher_drovi_bp = runTest(topGO_drovi_bp, algorithm = "classic", statistic = "fisher")
resultFisher_drovi_bp

resultKS_drovi_bp = runTest(topGO_drovi_bp, algorithm = "classic", statistic = "ks")
resultKS.elim_drovi_bp = runTest(topGO_drovi_bp, algorithm = "elim", statistic = "ks")

#analysis
allRes_drovi_bp = GenTable(topGO_drovi_bp, classicFisher = resultFisher_drovi_bp, 
                           classicKS = resultKS_drovi_bp, elimKS = resultKS.elim_drovi_bp,
                           orderBy = "elimKS", ranksOf = "classicFisher", topNodes = 25)

allRes_drovi_bp

pValue.classic_drovi = score(resultKS_drovi_bp)
pValue.elim_drovi = score(resultKS.elim_drovi_bp)[names(pValue.classic_drovi)]
gstat_drovi = termStat(topGO_drovi_bp, names(pValue.classic_drovi))
gSize_drovi = gstat_drovi$Annotated / max(gstat_drovi$Annotated) * 4
...
gCol_drovi = colMap(gstat_drovi$Significant)
plot(pValue.classic_drovi, pValue.elim_drovi, xlab = "p-value classic", ylab = "p-value elim",
     pch = 19, cex = gSize_drovi, col = gCol_drovi)

sel.go_drovi = names(pValue.classic_drovi)[pValue.elim_drovi < pValue.classic_drovi]
cbind(termStat(topGO_drovi_bp, sel.go_drovi),
      elim = pValue.elim_drovi[sel.go_drovi],
      classic = pValue.classic_drovi[sel.go_drovi])

showSigOfNodes(topGO_drovi_bp, score(resultKS.elim_drovi_bp), firstSigNodes = 5, useInfo = 'all')

#Molecular function
topGO_drovi_mf = new("topGOdata", 
                     ontology = "MF", 
                     allGenes = all_genes, 
                     geneSel = genes_modif,
                     nodeSize = 10, 
...
#enrichment test
resultFisher_drovi_mf = runTest(topGO_drovi_mf, algorithm = "classic", statistic = "fisher")
resultFisher_drovi_mf

resultKS_drovi_mf = runTest(topGO_drovi_mf, algorithm = "classic", statistic = "ks")
resultKS.elim_drovi_mf = runTest(topGO_drovi_mf, algorithm = "elim", statistic = "ks")

#analysis
allRes_drovi_mf = GenTable(topGO_drovi_mf, classicFisher = resultFisher_drovi_mf, 
                           classicKS = resultKS_drovi_mf, elimKS = resultKS.elim_drovi_mf,
                           orderBy = "elimKS", ranksOf = "classicFisher", topNodes = 25)
